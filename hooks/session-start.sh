...
if show_current_task; then
    ...
    echo ""
    echo "可用命令:"
    echo "  /dev-workflow  - 启动完整开发工作流"
    echo ""
    echo "可用 Agent:"
    echo "  requirement-analyst  - 需求分析"
    echo "  context-explorer     - 上下文调研"
    echo "  impact-analyst       - 影响分析"
    echo "  architect            - 方案设计"
    echo "  code-implementer     - 代码实现"
    echo "  code-reviewer        - 代码审核"
    echo "  test-engineer        - 测试工程"
    echo "  learning-recorder    - 学习记录"
    echo ""
    echo "**需求理解不清晰，坚决不写代码！**"
else
